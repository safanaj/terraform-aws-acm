locals {
  ...
  validation_domains = var.create_certificate && length(aws_acm_certificate.this) == 1 ? [for k, v in aws_acm_certificate.this[0].domain_validation_options : tomap(v) if contains(local.distinct_domain_names, replace(v.domain_name, "*.", ""))] : []
}
